 ...
 * @param bool $die 是否终止PHP
  * @param bool|string bool true=显示SYSTEM_NAME|bool false=不显示系统名|string=自定义系统名称
 */
function msg($msg = '未知的异常',$url = true,$die = true,$title = true) {
 ...
 <!DOCTYPE html>
    ...
    <body id="error-page">
      <h3><?php if($title===true){echo SYSTEM_NAME;}elseif($title===false){echo "System-Alarm";}else{echo $title;} ?> - 提示信息</h3><br/>
        <?php 
		echo $msg; 
               if ($url !== false) {
            if ($url === true) {
                ...
                echo '<br/><a style="float:right" href="'.$url.'">>> 前往操作</a>';
            }
        } 
		?>
        <br/>
